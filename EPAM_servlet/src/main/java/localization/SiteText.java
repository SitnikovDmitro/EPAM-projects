...
public class SiteText {
    private static SiteText instance;
    ...
    private SiteText() {
        ...
        enMap.put("session_expired", "Session expired. Please login again to continue the work");
        ...
        ruMap.put("session_expired", "Срок действия сессии истек. Чтобы продолжить работу войдите в систему заново");
        enMap.put("email_already_taken", "Этот почтовый адрес уже занят");
     ...
     */
    public String getText(String key, Lang lang) {
        String result = lang==Lang.RU?ruMap.get(key):enMap.get(key);
        if (result==null) return "Text not found for : "+key;
        return result;
    }

    /**
     * Returns translated order state
     * @param state state of order
     * @param lang target language to translate
     * @return translated state phrase
     */
    public String getStateText(OrderState state, Lang lang) {
        if (lang == Lang.RU) {
            return switch (state) {
                case PAID -> "Заплачено";
    ...
    public String getFormattedBalance(User user) {
        return new DecimalFormat("######.##").format(user.getBalance()/100.0);
    }

    public static SiteText getInstance() {
        if (instance == null) instance = new SiteText();
        return instance;
    }
}
