  ...
  <body>
    ...
    <div class="modal fade" id="finishChequeModal" tabindex="-1" aria-labelledby="modalLabel3" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">${text.translate('Cancel', lang)}</button>
            <a type="button" class="btn btn-primary" href="${pageContext.request.contextPath}/cashier/finishCheque">${text.translate('Finish', lang)}</a>
          </div>
        </div>
      </div>
    </div>
          ...
          <c:choose>
            <c:when test="${not empty chequeLines}">
              ...
              <div class="card-body">
                ...
                <a type="button" class="btn btn-primary" href="${pageContext.request.contextPath}/cashier/showProducts">${text.translate('Add next product', lang)}</a>
              </div>
            </c:when>
            <c:otherwise>
              <div class="card-body">
                <a type="button" class="btn btn-primary" href="${pageContext.request.contextPath}/cashier/showProducts">${text.translate('Add first product', lang)}</a>
              </div>
            </c:otherwise>
          </c:choose>
        </div>
    ...
    <script>
      function completeModal1() {
        ...
        fetch("${pageContext.request.contextPath}/cashier/removeProductFromCheque", { method: "POST", body: data }).then(
          response => response.json()
        ).then(
          data => {
            var result = data.result;
            if (result) {
              window.location.href = "${pageContext.request.contextPath}/cashier/showCheque";
            } else {
      ...
      function completeModal2() {
        ...
        fetch("${pageContext.request.contextPath}/cashier/removeCheque", { method: "POST", body: data }).then(
          response => response.json()
        ).then(
          data => {
            var result = data.result;
